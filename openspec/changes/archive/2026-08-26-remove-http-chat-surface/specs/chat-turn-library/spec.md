...
### Requirement: Auto-registered chat routes and pages

**Reason**: The package shipped an opinionated HTTP surface — fourteen routes, cookie and session conversation state, Inertia page rendering, and an SSE response wrapper — that no host could reuse under a different transport or UI, while forcing an Inertia dependency on every installation.

**Migration**: Hosts write their own controller and routes, calling `AiChatBotConversationService::startConversation()` and `continueConversation()`. The documented wire format is preserved by passing the yielded events through `SseFrameEncoder`. The published TypeScript client keeps working against a host endpoint that does so. Conversation resolution across requests — previously a session and the `ai_chat_bot_current` cookie — becomes the host's concern; `AiConversation::findByChatHashOrUuid()` is the lookup.

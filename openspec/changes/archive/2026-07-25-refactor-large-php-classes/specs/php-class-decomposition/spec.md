...
The chat-bot pages SHALL render the same component names with the same prop keys and values as before the refactor, including the differences that already exist between `show()` and `showByHash()`.

#### Scenario: Chat bot page rendered from session state

- **WHEN** `show()` renders `ai/ChatBot`
...
- **WHEN** `showByHash()` renders `ai/ChatBot`
...
- **WHEN** `index()` renders `ai/ChatBotsIndex`

...
### Requirement: Admin HTTP routes and controllers

**Reason**: The package rendered sixteen Inertia components it never shipped, documented, or typed, making the routes unusable without a bespoke host UI built against an undocumented interface. Management now happens through services the host drives from its own controllers.

**Migration**: Hosts keep their existing components and rewrite their controllers against `Jvjvjv\CodeTalker\Services\Management\*`. The route file, the `code-talker-admin-routes` publish tag, and the admin form request classes are removed; the validation rules they carried are available from the corresponding manager.
